    ...
    for (i = 0; i < dataSize;) {
        ...
        if (useProgMem) {
            ...
            for (j = 0; j < chunkSize; j++)
            {
                ...
                flash.read(&progBuffer[j], (unsigned long)(data + i + j), 1);
            }
        } else {
            // write the chunk of data as specified
...
bool MPU9250::writeDMPConfigurationSet(const uint8_t *data, uint16_t dataSize, bool useProgMem) {
    ...
    for (i = 0; i < dataSize;) {
        if (useProgMem) {
            ...
            flash.read(&bank, (unsigned long)(data + i++), 1);
            // offset = pgm_read_byte(data + i++);
            flash.read(&offset, (unsigned long)(data + i++), 1);
            // length = pgm_read_byte(data + i++);
            flash.read(&length, (unsigned long)(data + i++), 1);
        } else {
            bank = data[i++];
            offset = data[i++];
            length = data[i++];
        ...
        if (length > 0) {
            ...
            if (useProgMem) {
                ...
                flash.read(&special, (unsigned long)(data + i++), 1);
            } else {
                special = data[i++];
            }
